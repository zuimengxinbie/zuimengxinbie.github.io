...
此时的继承类似于 ES5 的寄生组合式继承，子类的原型对象中没有父类的实例对象和实例方法
...
除此之外，super 当做一个对象使用，在子类普通方法（原型对象）中，指向父类的原型对象，因此可以调用父类的原型方法，需要注意的是执行父类的原型方法时，在方法中执行时 this 指向的是子类的实例对象而不是父类的实例对象
